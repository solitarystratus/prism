import { loadSave, persistSave } from './core/storage.js';
...
(() => {
  ...
  function buildLevel(index) {
    ...
    level = {
      ...
      stats: { landed:new Set(), springBounces:0, bursts:0, beacons:0, goldenClouds:0, cloudBonusDrops:0 }
    };
    player = {
  ...
  function finishLevel() {
    ...
    const completionDrops = 70 + starsEarned * 30 + completedBonus * 20 + (level.stats.cloudBonusDrops || 0);
      ...
      if (dx*dx + dy*dy < player.r*player.r) {
        if (prevY + player.r <= p.y + 10 && player.vy >= 0) {
          ...
          if (p.type === 'golden' && !p.activated) {
            ...
            level.stats.cloudBonusDrops += 20;
  ...
  function draw(t,dt) {
    ...
    drawSky(t);
    drawRain(dt,t);
    if(level){drawCurrents(t);drawPortal(t);drawPlatforms(t);drawCheckpoints(t);drawStorms(t);drawBeacons(t);drawPrisms(t);drawRarePrisms(t);drawParticles();drawPlayer(t);}
