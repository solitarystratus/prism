(() => {
  ...
  function generateEndlessLevel(index) {
    ...
    const variant = tier % 5;
    const width = 4800 + (tier % 3) * 220;
    const goal = 10 + (tier % 3);
    ...
    const patternY = i => {
      if (variant === 0) return 455 + Math.sin(i * .9) * 62;
      if (variant === 1) return 500 - (i % 6) * 27;
      if (variant === 2) return [505, 462, 410, 455, 500, 448][i % 6];
      if (variant === 3) return 468 + Math.sin(i * .68) * 44;
      return [510, 445, 485, 410, 455, 500][i % 6];
    };

    while (cursor < width - 700) {
      const gap = 72 + Math.floor(rng() * 48);
      const w = 185 + Math.floor(rng() * 125);
      const targetY = patternY(step);
      const y = Math.round(clamp(targetY, Math.max(360, lastY - 72), Math.min(520, lastY + 72)));
      const x = cursor + gap;
      const springEvery = variant === 3 ? 3 : 4;
      const type = (step + 1) % springEvery === 0 ? 'spring' : 'cloud';
    ...
    for (let i = 0; i < goal; i++) {
      const idx = Math.min(usablePlatforms.length - 1, Math.floor((i + 1) * usablePlatforms.length / (goal + 1)));
      const p = usablePlatforms[idx];
      prisms.push([
        Math.round(p[0] + p[2] * (.35 + rng() * .3)),
        Math.round(p[1] - 78 - rng() * 24)
      ]);
    }
    ...
    const cloudTarget = 4 + (tier % 2);
    const burstTarget = 1 + (tier % 2);
    const pool = [
      { type:'cloudLandings', target:cloudTarget, label:'Cloud Hopper', short:`LAND ON ${cloudTarget} CLOUDS` },
      { type:'springBounces', target:1, label:'Spring Rider', short:'1 SPRING BOUNCE' },
      { type:'bursts', target:burstTarget, label:'Rainbow Rush', short:`${burstTarget} RAINBOW BURST${burstTarget > 1 ? 'S' : ''}` }
    ];
    const objectiveCount = tier >= 6 ? 3 : 2;
    const objectives = [];
    for (let i = 0; i < objectiveCount; i++) objectives.push({ ...pool[(variant + i) % pool.length] });
  ...
  function buildLevel(index) {
    ...
    level = {
      ...
      stats: { landed:new Set(), springBounces:0, bursts:0 }
    };
    player = {
  ...
  function finishLevel() {
    ...
    ui.rewardName.textContent = level.objectives.length ? `${level.reward} • ${bonusSummary()}` : level.reward;
    ui.levelEyebrow.textContent = state.levelIndex >= LEVELS.length ? 'ENDLESS SKY RESTORED' : 'SKY RESTORED';
