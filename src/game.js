(() => {
  ...
  const AURA_REWARDS = [
    { drops:600, key:'sunshower', name:'Sunshower Aura' },
    { drops:1500, key:'golden', name:'Golden Rain Aura' },
    { drops:3000, key:'aurora', name:'Aurora Aura' },
    { drops:6000, key:'prismatic', name:'Prismatic Aura' }
  ];
  ...
  function checkAuraUnlocks() {
    let newest = null;
    for (const reward of AURA_REWARDS) {
      if (save.drops < reward.drops || save.auras.includes(reward.key)) continue;
      save.auras.push(reward.key);
      save.equippedAura = reward.key;
      newest = reward.name;
    }
    return newest;
  }
  ...
  function finishLevel() {
    ...
    const auraUnlocked = checkAuraUnlocks();
    persist();
    ...
    if (auraUnlocked) extras.push(`${auraUnlocked} UNLOCKED`);
