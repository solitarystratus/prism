(() => {
  ...
  let save = {
    unlocked: 1, totalPrism: 0, best: {}, reward: 'silver', stars: {}, drops: 0,
    rare: { golden: 0, aurora: 0 }, rareClaims: {}, auras: ['mist'], equippedAura: 'mist',
    trails: ['silver'], equippedTrail: 'silver'
  };
  try {
  ...
  function updateShop() {
    ...
    ui.shopItems.innerHTML = renderShopSection('aura','AURAS') + renderShopSection('trail','TRAILS');
  ...
  function openShop(fromOverlay) {
    ...
    [ui.menu, ui.pause, ui.level, ui.finish, ui.shop].filter(Boolean).forEach(el => el.classList.remove('visible'));
    updateShop();
  ...
  const ENDLESS_REWARDS = ['Nimbus Token', 'Prism Bloom', 'Rainlight Crest', 'Cloudstep Charm', 'Aurora Drop'];
  const ENDLESS_HUES = [210, 228, 198, 246, 218, 190, 236, 204];
  const ARCHETYPES = [
  ...
  // Rainbow Drops are a spendable currency. Existing aura unlocks from older saves are preserved.
  const SHOP_ITEMS = [
    { kind:'aura', key:'mist', name:'Mist Aura', price:0, desc:'The original soft rain glow.' },
    { kind:'aura', key:'sunshower', name:'Sunshower Aura', price:400, desc:'A soft cyan halo that pulses around your raindrop.' },
    { kind:'aura', key:'golden', name:'Golden Rain Aura', price:900, desc:'Warm golden light with a brighter storm glow.' },
    { kind:'aura', key:'aurora', name:'Aurora Aura', price:1800, desc:'A shifting spectral halo inspired by aurora rain.' },
    { kind:'aura', key:'prismatic', name:'Prismatic Aura', price:3200, desc:'The rarest full-spectrum aura.' },
    { kind:'trail', key:'silver', name:'Silver Drizzle Trail', price:0, desc:'The original cool mist trail.' },
    { kind:'trail', key:'rainbow', name:'Rainbow Trail', price:550, desc:'Leaves a shifting rainbow wake while you move.' },
    { kind:'trail', key:'golden', name:'Golden Trail', price:1100, desc:'A luminous gold trail behind every jump.' },
    { kind:'trail', key:'aurora', name:'Aurora Trail', price:2100, desc:'A cool color-shifting trail with a softer glow.' },
    { kind:'trail', key:'prismatic', name:'Prismatic Trail', price:3400, desc:'A vivid spectral wake for long-term collectors.' }
  ];
  ...
  function makePrimary(type, tier) {
  ...
  function generateEndlessLevel(index) {
    const levelNumber = index + 1;
    const tier = index - LEVELS.length;
    const rng = seededRandom(0x9E3779B9 ^ Math.imul(levelNumber, 0x85EBCA6B));
    const archetype = ARCHETYPES[tier % ARCHETYPES.length];
    const primaryType = ['prisms','beacons','cloudLandings','springBounces'][tier % 4];
    const primary = makePrimary(primaryType, tier);
    const width = 4800 + (tier % 3) * 220;
    ...
    while (cursor < width - 700) {
      ...
      const x = cursor + gap;
      const type = (step + 1) % archetype.springEvery === 0 ? 'spring' : 'cloud';
      platforms.push([x, y, w, type === 'spring' ? 42 : 40, type]);
      cursor = x + w;
      lastY = y;
      step++;
    }
    ...
    const normalPrismCount = primary.type === 'prisms' ? primary.target : 7 + (tier % 2);
    const prisms = [];
    for (let i = 0; i < normalPrismCount; i++) {
      const idx = Math.min(usablePlatforms.length - 1, Math.floor((i + 1) * usablePlatforms.length / (normalPrismCount + 1)));
    ...
    ].filter(o => o.type !== primary.type);
    const objectiveCount = tier >= 8 ? 3 : 2;
    const objectives = [];
    for (let i = 0; i < objectiveCount; i++) objectives.push({ ...bonusPool[(tier + i) % bonusPool.length] });

    return {
      name: `${ENDLESS_NAMES[tier % ENDLESS_NAMES.length]} ${levelNumber}`,
      hue: ENDLESS_HUES[tier % ENDLESS_HUES.length],
      goal: normalPrismCount,
      reward: ENDLESS_REWARDS[tier % ENDLESS_REWARDS.length],
      width,
      start: [160, 420],
  ...
  function getLevelSpec(index) {
    return index < LEVELS.length ? LEVELS[index] : generateEndlessLevel(index);
  }
  ...
  function buildLevel(index) {
    ...
    level = {
      ...
      rarePrisms: (spec.rarePrisms || []).map((p,i) => ({ x:p[0], y:p[1], kind:p[2], r:15, got:false, phase:i*1.3 + .4 })),
      ...
      stats: { landed:new Set(), springBounces:0, bursts:0, beacons:0 }
    };
    player = {
    ...
    state.rainIntensity = .75 + Math.min(index, LEVELS.length - 1) * .08;
    state.cameraX = 0;
    state.cameraY = 0;
    particles = [];
    rings = [];
  ...
  function claimRareRewards(levelNumber) {
    let bonus = 0;
    const found = [];
    level.rarePrisms.forEach((gem, i) => {
      if (!gem.got) return;
      const claimId = `${levelNumber}:${gem.kind}:${i}`;
      if (save.rareClaims[claimId]) return;
      save.rareClaims[claimId] = true;
      const amount = gem.kind === 'aurora' ? 250 : 75;
      bonus += amount;
      save.rare[gem.kind] = (save.rare[gem.kind] || 0) + 1;
      found.push(gem.kind === 'aurora' ? 'Aurora Prism' : 'Golden Prism');
    });
    return { bonus, found };
  }
  ...
  function startGame(index = 0) {
    ...
    setTimeout(() => {
      if (state.scene === 'playing' && state.levelIndex === startedLevel && !state.paused) {
        const prefix = state.levelIndex >= LEVELS.length ? `${level.archetype.name} • ` : '';
  ...
  function finishLevel() {
    ...
    const completionDrops = 70 + starsEarned * 30 + completedBonus * 20;
    const rareReward = claimRareRewards(number);
    state.levelDropsEarned = completionDrops + rareReward.bonus;
    ...
    const extras = [];
    if (rareReward.found.length) extras.push(rareReward.found.join(' + '));
  ...
  function update(dt) {
    ...
    for (const gem of level.rarePrisms) {
      if (gem.got) continue;
      const dx = player.x-gem.x, dy = player.y-gem.y;
      if (dx*dx + dy*dy < (player.r+gem.r+20)**2) {
        gem.got = true;
        player.burst = clamp(player.burst + .28,0,1);
        state.flash = .28;
        const aurora = gem.kind === 'aurora';
        audio.tone(aurora ? 760 : 640, .28, 'triangle', .42, aurora ? 460 : 280);
        for (let i=0;i<28;i++) addParticle(gem.x,gem.y,{type:'prism',hue:aurora ? (i/28)*360 : 45,life:rand(.45,.9),r:rand(2,6)});
        rings.push({x:gem.x,y:gem.y,r:10,life:.7,max:.7});
        toast(aurora ? 'RARE AURORA PRISM FOUND' : 'GOLDEN PRISM FOUND');
      }
    }
  ...
  function drawRarePrisms(t) {
    for (const gem of level.rarePrisms) {
      if (gem.got) continue;
      const [x,y0]=worldToScreen(gem.x,gem.y); const y=y0+Math.sin(t*.0045+gem.phase)*8;
  ...
  function draw(t,dt) {
    ...
    drawSky(t);
    drawRain(dt,t);
    if(level){drawCurrents(t);drawPortal(t);drawPlatforms(t);drawStorms(t);drawBeacons(t);drawPrisms(t);drawRarePrisms(t);drawParticles();drawPlayer(t);}
  ...
  ui.nextBtn.addEventListener('click',()=>startGame(state.levelIndex+1));
