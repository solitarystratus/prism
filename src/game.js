...
(() => {
  ...
  function buildLevel(index) {
    ...
    level = {
      ...
      rarePrisms: (spec.rarePrisms || []).map((p,i) => {
        ...
        return { x:p[0], y:p[1], kind:p[2], r:15, got:!!save.rareClaims[claimId], claimId, phase:i*1.3 + .4 };
      }),
      ...
      stats: { landed:new Set(), springBounces:0, bursts:0, beacons:0, goldenClouds:0, cloudBonusDrops:0, obstacleHits:0 }
    };
    player = {
      x: spec.start[0], y: spec.start[1], vx:0, vy:0, r:18,
      grounded:false, coyote:0, jumpBuffer:0, burst:1, burstCooldown:0,
      invuln:0, shield:save.rarePerks.auroraTier >= 3 ? 2 : save.rarePerks.auroraTier >= 1 ? 1 : 0, trail:[], facing:1, squash:1, stretch:1
    };
  ...
  function draw(t,dt) {
    ...
    drawSky(t);
    drawRain(dt,t);
    if(level){drawCurrents(t);drawPortal(t);drawPlatforms(t);drawCheckpoints(t);drawObstacles(t);drawStorms(t);drawBeacons(t);drawPrisms(t);drawRarePrisms(t);drawParticles();drawPlayer(t);}
